...
namespace CodeEasier.Polish
{
    ...
    */
    ...
    class CEParticleEmitter
    {
        ...
        public void AddParticle(Texture2D texture, int x, int y, int w, int h, float timer, float vx, float vy, float vAngle)
        {
            CEParticle p = new CEParticle(texture, x, y, w, h, timer, vx, vy, vAngle);
            Particles.Add(p);
        }
        ...
        public virtual void Draw(SpriteBatch spriteBatch)
        {
            foreach (CEParticle p in Particles)
            {
                spriteBatch.Draw(p.Texture, p.Rect, null, Color.White * p.Alpha, MathHelper.ToRadians(p.Angle), p.Origin, SpriteEffects.None, 0);
            }
        }
