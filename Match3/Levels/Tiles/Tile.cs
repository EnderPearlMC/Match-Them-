...
namespace Match3.Levels.Tiles
{
    abstract class Tile
    {
        ...
        public virtual void Update(SceneLevel scene, GameTime gameTime, Main BaseGame)
        {
            ...
            if (isTileMoving)
            {
                ...
                if (movingDirection != "")
                {
                    ...
                    {
                        XOff -= dt * (BaseGame.game.ScreenWidth / 2.3f);
                    ...
                    {
                        if (speedDown < BaseGame.game.ScreenHeight / 3)
                            speedDown += BaseGame.game.ScreenHeight / 130;
                        YOff += dt * speedDown;
                        // Verify if the tile has reached its position
                        if (Y >= positionToGo.Y)
                        {
            ...
            if (!scene.isPaused && activated && !scene.won && !scene.lost)
            {
                                    ...
                                    if (row < scene.tiles.Count - 1)
                                    {
                                        if (scene.tiles[row + 1][col] != null)
                                        {
                                            if (diff == -1)
                                            {
                                                ...
                                                scene.showIndicator = true;
                                                scene.indicatorRow = mouseRow;
                                                scene.indicatorCol = mouseCol;
            ...
            */
            ...
            gravityCounter += (float) gameTime.ElapsedGameTime.TotalSeconds;
            ...
            if (row < scene.tiles.Count - 1 && row >= 0)
            {
                if (col >= 0 && col < scene.tiles[row].Count)
                {
                    ...
                    /**if (scene.tiles[row + 1][col] != null && scene.tiles[row][col] != null)
                    {
                        if (!scene.tiles[row][col].isTileMoving && !scene.tiles[row][col].toBeStacked)
                        {
                            if (gravityCounter > 0.2f && !gravitySounded)
                            {
                                Assets.SndTileSlide.Play();
                                gravitySounded = true;
                            }
        ...
        public virtual void Draw(SpriteBatch spriteBatch)
        {
            ...
            if (wrong)
                c = new Color(194, 54, 22);
            if (!activated)
                alpha = 0.5f;

            selected = false;

            spriteBatch.Draw(Texture, new Rectangle(X, Y, Width, Height), c * alpha);
        }

        /**
         *  Event : When the tile is matched
         */
        public virtual void OnMatch(SceneLevel scene)
        {
