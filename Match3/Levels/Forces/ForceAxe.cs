...
namespace Match3.Levels.Forces
{
    class ForceAxe : Force
    {
        ...
        public override void Update(GameTime gameTime, SceneLevel scene)
        {
            ...
            {
                axeRect = new Rectangle(tileToBreak.X + ((int)Math.Round(axeRect.Width * 1.6)), tileToBreak.Y + ((int)Math.Round(axeRect.Height / 2.5)), BaseGame.game.ScreenWidth / 13, BaseGame.game.ScreenHeight / 7);
            ...
            if (toBreak)
            {
                angle -= (float)gameTime.ElapsedGameTime.TotalSeconds * (-angle + 100);
                ...
                if (angle <= -50)
                {
                    ...
                    CEParticle pw = new CEParticle(Assets.FXWaveCircle, tileToBreak.X + tileToBreak.Width / 2, tileToBreak.Y + tileToBreak.Height / 2, tileToBreak.Width / 3, tileToBreak.Height / 3, 2, 0, 0, 0);
                    pw.AlphaAmount = .5f;
                    pw.GrowingAmount = 10;
                    pw.Center = true;
                    ...
                    scene.AddCircleParticle(15, tileToBreak.X + tileToBreak.Width / 2, tileToBreak.Y + tileToBreak.Height / 2, tileToBreak.Width / 4, tileToBreak.Height / 4, 2, tileToBreak.Width / 3, tileToBreak.Width * 7, false, new Color(255, 204, 0), true);
                    scene.AddCircleParticle(25, tileToBreak.X + tileToBreak.Width / 2, tileToBreak.Y + tileToBreak.Height / 2, tileToBreak.Width / 4, tileToBreak.Height / 4, 2, tileToBreak.Width, tileToBreak.Width * 8, false, new Color(255, 204, 0), true);
