    ...
    class EmitterXP : CEParticleEmitter
    {
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            foreach(CEParticle p in Particles)
            {
                ...
                p.Rect = new Rectangle((int) Math.Round(Utils.EaseOutSin(time, beginx, distancex, duration)), (int) Math.Round(Utils.EaseInOutQuart(time, beginy, distancey, duration)), p.Rect.Width, p.Rect.Height);
                ...
                if (r.Next(0, 100) < 50)
                {
                    ...
                    if (r.Next(0, 100) < 50)
                    {
                        c = new Color(76, 209, 55);
                    }
                    else
                    {
                        c = new Color(39, 174, 96);
                    }
                    ...
                    CEParticle xpP = new CEParticle(rectangle, p.Rect.X - p.Rect.Width / 2, p.Rect.Y - p.Rect.Height / 2, width, width, (float)r.NextDouble() * 2, 0, r.Next(-60, 60), 200, 0, r.Next(500, 2000));

                    xpP.GrowingAmount = -1.4f;
