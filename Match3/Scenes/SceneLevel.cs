...
namespace Match3.Scenes
{
    class SceneLevel : CEScene
    {
        ...
        public override void Load(Dictionary<string, Object> parameters)
        {
            ...
            Song song = Assets.LevelMusics[r.Next(0, Assets.LevelMusics.Count)];

            if (BaseGame.game.oldSong != null)
            {
                while (song == BaseGame.game.oldSong)
                {
                    Random r2 = new Random();
                    song = Assets.LevelMusics[r.Next(0, Assets.LevelMusics.Count)];
                }
            }
        ...
        private void UpdateTiles(GameTime gameTime)
        {
            ...
            if (match4Circles)
            {
                ...
                if (r.Next(0, 100) < 10)
                {
                    ...
                    CEParticle ps = new CEParticle(Assets.FXSmoke1, r.Next(0, BaseGame.game.ScreenWidth), BaseGame.game.ScreenHeight, r.Next(BaseGame.game.ScreenWidth / 5, BaseGame.game.ScreenWidth / 3), r.Next(BaseGame.game.ScreenHeight / 5, BaseGame.game.ScreenHeight / 3), 2, 0, r.Next(-300, -200), 0);

                    ps.AlphaAmount = .5f;

                    emitter1.AddParticle(ps);
                                ...
                                if (!tile1.isTileMoving && !tile2.isTileMoving && !tile3.isTileMoving && !tile1.toRemove && !tile2.toRemove && !tile3.toRemove)
                                {
                                    ...
                                    if (col < tiles[row].Count - 3)
                                    {
                                        if (tiles[row][col + 3] != null)
                                        {
                                            ...
                                            if (tile3.Id == tile4.Id)
                                            {
                                                tiles[row][col + 3].toRemove = true;
                                                tiles[row][col + 3].removeCounter = 1.6f;
                                    ...
                                    tiles[row][col].removeCounter = 0.7f;
                                    tiles[row][col + 1].removeCounter = 1f;
                                    tiles[row][col + 2].removeCounter = 1.3f;
                                    
                                    tiles[row][col + 1].OnMatch(this);
                                    
                                    float a = (float)LevelsData.LevelsList[BaseGame.game.Player.Level - 1].MaxXp / xp;

                                    CEParticle xpP = new CEParticle(Assets.FXXP, 0, 0, tiles[row][col + 1].Width / 2, tiles[row][col + 1].Height / 2, 3, 0, 0, 80);
                        ...
                        if (tile1.Id == tile2.Id && tile1.Id == tile3.Id && tile3.Id == tile2.Id)
                        {
                            if (!tile1.Numbered && !tile2.Numbered && !tile3.Numbered)
                            {
                                if (!tile1.isTileMoving && !tile2.isTileMoving && !tile3.isTileMoving && !tile1.toRemove && !tile2.toRemove && !tile3.toRemove)
                                {
                                    ...
                                    if (row < tiles.Count - 3)
                                    {
                                        if (tiles[row + 3][col] != null)
                                        {
                                            ...
                                            if (tile3.Id == tile4.Id)
                                            {
                                                ...
                                                tiles[row + 3][col].removeCounter = 1.6f;
                                    ...
                                    tiles[row][col].removeCounter = 0.7f;
                                    tiles[row + 1][col].removeCounter = 1f;
                                    tiles[row + 2][col].removeCounter = 1.3f;
                                    ...
                                    CEParticle xpP = new CEParticle(Assets.FXXP, 0, 0, tiles[row + 1][col].Width / 2, tiles[row + 1][col].Height / 2, 3, 0, 0, 80);
        ...
        public void AddText(Texture2D texture)
        {
            ...
            pt.Props.Add("distancex", -BaseGame.game.ScreenWidth * 1.4);
